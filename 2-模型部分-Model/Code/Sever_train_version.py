...
def LSTM_Model(X_train, Y_train,i):
    filepath = "E:\\Core\\EEMD-LSTM-imf" +str(i) +"-{epoch:02d}.h5"
...
def plot_curve(true_data, predicted):
    ...
    plt.legend()
    plt.text(1, 1, 'RMSE:' + str(format(RMSE(test,prediction),'.4f'))+' \n '+'MAPE:'+str(format(MAPE(test,prediction),'.4f')), color = "r",style='italic', wrap=True)
    ...
    plt.savefig('E:\\Core\\result_EEMD_LSTM.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    dataset = pd.read_csv('E:\\Core\\Water Quality Record.csv', header=0, index_col=0, parse_dates=True)
    ...
    plt.savefig('E:\\Core\\result_imf.png')
##    plt.show()
    plt.clf()
