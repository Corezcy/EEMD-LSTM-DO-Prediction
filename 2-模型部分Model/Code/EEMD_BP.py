...
def BP_Model(X_train, Y_train,i):
    filepath = '../EEMD-BP/EEMD-BP-imf'+str(i)+'-{epoch:02d}-16.h5'
    ...
    model.fit(X_train, Y_train, epochs=100, batch_size=16, validation_split=0.1, verbose=2, shuffle=True,callbacks=callbacks_list)
    return (model)
...
if __name__ == '__main__':
    ...
    for imf in imfs:
        ...
        test += Y2_test
        model = load_model('../EEMD-BP/EEMD-BP-imf'+str(i)+'-100-16.h5')
    ...
    #    # model.save('EEMD-BP-imf' + str(i) + '.h5')
    #    prediction_Y = model.predict(X2_test)
    #    imfs_prediction.append(prediction_Y)
    #    i+=1;
